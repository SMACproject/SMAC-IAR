/***********************************************************************************
Filename:  CC2530 Wireless Firmware Taktile Capsule
***********************************************************************************/
...
#define PC_ADDR              0xDECA
#define SENSOR_ADDR          0xBECC
...
unsigned int ADC1_int=0;
unsigned int ADC2_int=0;
unsigned int ADC3_int=0;
unsigned int ADC4_int=0;
unsigned int ADC5_int=0;
unsigned int ADC6_int=0;


unsigned char acc[6] = {0};
unsigned char acc_old[6] = {0};
...
int main(void){  
    ...
    SPI1_Switch_SSN(P1_4,HIGH);
    SPI1_Switch_SSN(P1_3,HIGH);
    SPI1_Switch_SSN(P1_2,HIGH);
    SPI1_Switch_SSN(P1_1,HIGH);
    SPI1_Switch_SSN(P1_0,HIGH);
...
//     if ((acc[0] != 255) && (acc[1] != 255) && (acc[2] != 255) &&  (acc[3] != 255) &&  (acc[4] != 255) &&  (acc[5] != 255)) {
//     //x  
//     pTxData[17] =  acc[0];
//     pTxData[18] =  acc[1];
//     //y
...
//     pTxData[22] =  acc[5];
//     
//     memcpy(acc_old,acc,sizeof(acc));
//     }
//        
//        else {  // To prevent the case the data of the accellerometer is not yet ready   :) 
//     //x 
//     pTxData[17] =  acc_old[0];
//     pTxData[18] =  acc_old[1];
//     //y
//     pTxData[19] =  acc_old[2];
//     pTxData[20] =  acc_old[3];
//     //z
//     pTxData[21] =  acc_old[4];
//     pTxData[22] =  acc_old[5];
//     }
   ...
   retry:
     ...
     RF_Timeout_Status = RF_Timeout(4000);
     if (( basicRfReceive(pRxData, APP_PAYLOAD_LENGTH,NULL)  >0 ) && (RF_Timeout_Status == FALSE))  {
